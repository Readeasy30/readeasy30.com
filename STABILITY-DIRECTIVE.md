...
This directive establishes the standard operating rule for this repository and all future AI-assisted work.
...
4. Future changes must preserve the working site or application.
5. Any fix must be recorded so the same issue is not rediscovered later.
6. New features must not be added until the existing production path is stable.

In short: **lock = stabilize, protect, document, and verify.**

## Required behavior for AI agents and contributors
...
Do not start new feature work while the existing site or application is broken.

Priority order:

1. Restore the live production path.
2. Remove or isolate the cause of failure.
3. Confirm the correct files are in the repository, not only in a direct deployment.
4. Record the fix in status or changelog files.
5. Add a guardrail if the same mistake could happen again.
6. Only then continue new production work.
...
Do not add the following without direct approval:
...
- what still needs browser or deployment testing

## Release rule

A repo is not considered stable merely because files were edited. It is stable when:

1. The intended production path is clear.
2. Conflicting old files are removed or documented.
3. The site or app has a defined browser QA path.
4. Deployment settings are known or documented.
5. Future agents know what not to undo.
