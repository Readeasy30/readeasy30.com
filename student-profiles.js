(() => {
  ...
  function buildProfileCards(profiles, activeId) {
    return profiles.map(profile => {
      const stats = getReadingStats(profile.id);
  ...
  function interceptResetClicks() {
    document.addEventListener("click", event => {
      ...
      const isReset = button.matches(CONFIG.resetSelector) || text === "reset this student" || text === "reset progress on this device";
      if (!isReset) return;
  ...
  window.ReadEasy30StudentProfiles = {
